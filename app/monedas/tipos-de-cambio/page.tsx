...
export default function TiposDeCambioPage() {
  ...
  const shouldShowBuySell = (rate: ExchangeRate) => {
    return rate.compra && rate.venta && Math.abs(rate.compra - rate.venta) > 0.01
  }
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="text-center space-y-4">
          <h1 className="text-4xl font-bold tracking-tight text-foreground">Tipos de Cambio</h1>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={fetchExchangeRates} disabled={loading}>
          <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""} max-[360px]:mr-0 mr-2`} />
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Cotizaciones {usingFallback ? "de referencia" : "actualizadas en tiempo real"} desde Argentina
        </p>
        ...
        {usingFallback && (
          <div className="bg-yellow-100 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-lg p-4 max-w-2xl mx-auto">
            <p className="text-sm text-yellow-800 dark:text-yellow-200">
              ⚠️ Usando cotizaciones {lastSuccessfulUpdate ? "de cache" : "de referencia"}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {exchangeRates.map((rate, index) => (
            <Card key={`${rate.moneda}-${rate.casa}-${index}`} className="hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                {/* Layout Desktop (>640px) */}
                <div className="hidden sm:flex items-center justify-between">
                  {/* Lado izquierdo - Info de la moneda */}
                  <div className="flex items-center space-x-3">
                    <div className="text-xl">{getCurrencyIcon(rate.moneda)}</div>
                    <div className="min-w-0 flex-1">
                      <h3 className="text-lg font-semibold truncate">
                        {rate.nombre}
                        {rate.casa && <span className="text-xs text-muted-foreground ml-1">({rate.casa})</span>}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        1 {rate.moneda} = ${(rate.venta || rate.compra).toFixed(2)} ARS
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    {shouldShowBuySell(rate) ? (
                      ...
                      <div className="space-y-1 w-full">
                        <div className="flex items-center justify-center space-x-4 text-sm">
                          <div className="text-center">
                            ...
                            <p className="text-lg font-semibold text-green-600">${rate.compra.toFixed(2)}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-sm text-muted-foreground">Venta</p>
                            <p className="text-lg font-semibold text-red-600">${rate.venta.toFixed(2)}</p>
                          </div>
                        </div>
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xl font-bold text-primary">${(rate.venta || rate.compra).toFixed(2)}</p>
                      </div>
                    )}
                    <p className="text-sm text-muted-foreground mt-1">
                      {rate.fechaActualizacion && formatDate(new Date(rate.fechaActualizacion))}
                    </p>
                  </div>
                </div>
                    ...
                    {shouldShowBuySell(rate) ? (
                      <>
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-lg font-semibold text-green-600">${rate.compra.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-muted-foreground">Venta:</span>
                          <span className="text-lg font-semibold text-red-600">${rate.venta.toFixed(2)}</span>
                        </div>
        ...
        <Card className="bg-muted/30">
          <CardContent className="p-6">
            <div className="text-center space-y-2">
              <h3 className="font-semibold">Información sobre los tipos de cambio</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-base text-muted-foreground">
                <div>
                  <p>
                    <strong>Compra:</strong> Precio al que el banco/casa de cambio compra la moneda extranjera
                  </p>
                </div>
                <div>
                  <p>
                    <strong>Venta:</strong> Precio al que el banco/casa de cambio vende la moneda extranjera
                  </p>
                </div>
                <div>
                  <p>
                    <strong>Promedio:</strong> Precio medio entre compra y venta para referencia rápida
                  </p>
                </div>
              </div>
              <div className="pt-2 border-t">
                <p className="text-base">
                  <strong>Actualización:</strong> {lastUpdated ? formatDate(lastUpdated) : "Cargando..."}
                </p>
                <p className="text-base">
                  <strong>Fuente:</strong> {usingFallback ? "Datos de cache/referencia" : "DolarAPI.com"}
                </p>
                {lastSuccessfulUpdate && usingFallback && (
                  <p className="text-base">
                    <strong>Última actualización exitosa:</strong> {formatDate(lastSuccessfulUpdate)}
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="text-center space-y-4 py-8">
        ...
        <p className="text-muted-foreground">Usa nuestros conversores para calcular equivalencias al instante</p>
